...
// ========================================================================
// --- SCENE 0: DESKTOP & GLOBAL SETUP ---
// ========================================================================

const desktopEnvironment = document.getElementById(
  "desktop-environment"
) as HTMLElement;
...
const readmeModalBackdrop = document.getElementById(
  "readme-modal-backdrop"
) as HTMLElement;
const closeReadmeBtn = document.getElementById(
  "close-readme-btn"
) as HTMLElement;
...
cortexIcon.addEventListener("click", () => {
    playDesktopSound('desktop_click.m4a');
  if (!isAudioUnlocked) {
    // We don't need to do anything with the context,
    // just creating it is enough to signal user interaction.
    new (window.AudioContext || (window as any).webkitAudioContext)();
    isAudioUnlocked = true;
  }
  desktopEnvironment.style.display = "none";
  readmeModalBackdrop.style.display = "none";
...
function initializeGame() {
  ...
  function updateOrganicState() {
    ...
    if (!container) return;
    const STAGE_2_THRESHOLD = 200;
    const STAGE_3_THRESHOLD = 400;
  ...
  function triggerExplosion() {
    ...
    const explosionCanvas = document.getElementById(
      "explosion-canvas"
